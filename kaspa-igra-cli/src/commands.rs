...
pub async fn generate_wallet() -> Result<WalletInfo> {
    ...
    let address = crate::wallet::generate_address(keypair.public_key(), Network::Testnet12);
...
pub async fn validate_address(address: &str) -> bool {
    crate::wallet::validate_address(address, Network::Testnet12).unwrap_or(false)
}
...
pub async fn generate_hd_wallet() -> Result<HDWalletInfo> {
    ...
    let address = crate::wallet::generate_address(address0.keypair().public_key(), Network::Testnet12);
...
pub async fn send_graffiti(
    ...
    let prefix = Network::Testnet12.to_prefix();
