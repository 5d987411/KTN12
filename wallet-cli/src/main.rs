...
enum Commands {
    ...
    /// Send KAS
    Send { private_key: String, recipient: String, amount: f64 },
...
fn main() {
    ...
    match cli.command {
        ...
        Commands::Load { private_key } => {
            let secp = Secp256k1::new();
            let secret_bytes = hex::decode(&private_key).expect("Invalid private key hex");
            ...
            let secret = secp256k1::SecretKey::from_slice(&secret_bytes).expect("Invalid private key");
        ...
        Commands::Send { private_key, recipient, amount } => {
            ...
            let url = format!(
                "https://api-tn12.kaspa.org/addresses/{}/utxos",
